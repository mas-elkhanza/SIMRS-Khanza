<?php
    ...
    if (isset($iyem["nopersetujuan"])) {
        ...
        $querypersetujuan = bukaquery(
            ...
            "surat_pernyataan_pasien_umum.jkpj,surat_pernyataan_pasien_umum.alamatpj,surat_pernyataan_pasien_umum.hubungan,surat_pernyataan_pasien_umum.no_telp,surat_pernyataan_pasien_umum_pembuat_pernyataan.photo ".
            "from surat_pernyataan_pasien_umum inner join surat_pernyataan_pasien_umum_pembuat_pernyataan on surat_pernyataan_pasien_umum.no_surat=surat_pernyataan_pasien_umum_pembuat_pernyataan.no_surat ".
            "where surat_pernyataan_pasien_umum.no_surat='".$nopersetujuan."'"
        );
        if($rsquerypersetujuan= mysqli_fetch_array($querypersetujuan)){
            $src = 'data: '.mime_content_type("http://".host()."/webapps/pernyataanumum/".$rsquerypersetujuan["photo"]).';base64,'.base64_encode(file_get_contents("http://".host()."/webapps/pernyataanumum/".$rsquerypersetujuan["photo"]));
