    ...
    if (isset($iyem["norawat"])) {
        ...
        if($rsqueryusgneonatus= mysqli_fetch_array($queryusgneonatus)){
            ...
            if($rsquerygambarusg= mysqli_fetch_array($querygambarusg)){
                $src = 'data: '.@mime_content_type("http://".host()."/webapps/hasilpemeriksaanusgneonatus/".$rsquerygambarusg["photo"]).';base64,'.base64_encode(file_get_contents("http://".host()."/webapps/hasilpemeriksaanusgneonatus/".$rsquerygambarusg["photo"]));
                echo "              <tr>
