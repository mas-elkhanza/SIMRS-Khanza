	        ...
	        <div class="modal-dialog modal-lg fade-left">
	            <div class="modal-content">
	                ...
	                <div class="modal-body">
	                    <form action="<?php echo base_url('ps_info_view/action/save') ?>" method="post" id="modForm" class="form-horizontal" enctype="multipart/form-data">
	                        ...
	                        <div class="col-md-12">
	                        	<div class="row">
                        			<div class="col-sm-6">
				                      ...
				                      <input type="datetime-local" name="waktu_masuk" id="waktu_masuk" class="form-control" value="<?php echo date('Y-m-d').'T'.date('H:i'); ?>">   
				                    </div>
				                    <div class="col-sm-6">
				                      <label>Tanggal Praktek</label>
				                      <input type="date" name="tgl_periksa" id="tgl_periksa" class="form-control" value="<?php echo date('Y-m-d'); ?>">   
				                    </div>
	                        	</div>
	                        </div>
	                        <div class="col-md-12">
		...
		<script>
			var link_url = '<?php echo base_url('ps_info_view/action');?>';
			if (!checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ps_info_view');?>')) {
		        history.go(-1);
		    }
		    ...
		    $(document).ready(function(){
			    ...
			    $('.tr_mod').click(function () {
		            if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>','<?php echo aksesName('ps_info_edit');?>')) {
		                var tr = $(this).parent();
		                ...
		                $('#FormBc [name="waktu_masuk"]').val(tr.data('waktu_masuk'));
		        ...
		        $('#poliklinik').change(function(){
		        	...
		        	var waktu_masuk = $('#waktu_masuk').val();

		        	$.ajax({
		        		type : 'post',
		        		url : link_url+'/get_data_booking_pasien',
		        		data : 'kd_poli='+kd_poli+'&kd_dokter='+kd_dokter+'&waktu_masuk='+waktu_masuk,
		        		success : function(r)
		        		{
		        			$('#tanggal_periksa').html(r);
		        		}
		    ...
		    function formAdd() {
		        if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ps_info_add');?>')) {
		            $('.sm-war-kosong').hide();
