				...
				<div class="row">
    				<div class="col-md-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <input type="text" class="form-control input-sm" name="cari" placeholder="Cari" value="<?php echo ($f!='f') ? $f : ''; ?>" onkeyup="event.keyCode == 13 ? window.location = '<?php echo base_url('ad_asuransi_view/action/view/'); ?>' + this.value : '';">
                            </div>
                        </div>
		                        ...
		                        <tbody>
		                        	<?php 
		                        		...
		                        		foreach ($r as $idrw => $rw) {
			                                ...
			                                echo '<td class="tr_mod">' . $rw->nm_asuransi. '</td>';
			                                echo '</tr>';
			                            }
		        ...
		        <div class="modal-dialog fade-left">
		            <div class="modal-content">
		                <div class="modal-header">
		                    <h4 class="modal-title">Manajemen Asuransi</h4>
		                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
		                </div>
		                <div class="modal-body">
		                    <form action="<?php echo base_url('ad_asuransi_view/action/save') ?>" method="post" id="modForm" class="form-horizontal" enctype="multipart/form-data">
		                        <input type="hidden" name="id">
		                        <div class="form-group">
		                            <label for="tun" class="col-sm-4 control-label">Nama Asuransi</label>

		                            <div class="col-sm-12">
		                                <input type="text" class="form-control input-sm" name="nm_asuransi" placeholder="Nama Asuransi" required="true">
		                            </div>
		                        </div>
		                    </form>
		                </div>
		...
		<script>
			if (!checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ad_asuransi_view');?>')) {
		        history.go(-1);
		    }
		    ...
		    $(document).ready(function () {
		        ...
		        $('.tr_mod').click(function () {
		            if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>','<?php echo aksesName('ad_asuransi_edit');?>')) {
		                var tr = $(this).parent();

		                $('#modalMod :input').val('');
		                $('#modalMod [name="id"]').val(tr.data('id'));
		                $('#modalMod [name="nm_asuransi"]').val(tr.data('nm_asuransi'));
		    ...
		    function formAdd() {
		        if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ad_asuransi_add');?>')) {
		            $('#modalMod :input').val('');
		            $('#modalMod [name="nm_asuransi"]').prop('required', true);
		            $('.sm-war-kosong').hide();
		    ...
		    function formDelete() {
		        if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ad_asuransi_delete');?>')) {
		            if (confirm('Are you sure want to delete this data?')) {
		                ...
		                window.location = '<?php echo base_url('ad_asuransi_view/action/delete/')?>' + (idC.join('k'));
		            }
		        }
		    }
		</script>