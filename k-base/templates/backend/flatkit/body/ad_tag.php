    				...
    				<div class="col-md-12">
		                        ...
		                        <tbody>
		                        	<?php 
		                        		...
		                        		foreach ($r as $idrw => $rw) {
			                                ...
			                                echo '<td class="tr_mod">' . $rw->nm_tag. '</td>';
			                                echo '<td class="tr_mod">' . $rw->tag_seo. '</td>';
			                                echo '</tr>';
			                            }
                        ...
                        <div class="row">
	                    	...
	                    	<div class="col-md-3 text-left">
		                        <?php echo 'Limit : ' . htmlSelectFromArray($limit, 'name="l" style="width:100px;" onchange="window.location=\'' . base_url('ad_tag_view/action/view/') . $f . '/\'+this.value"/0', true, $l); ?>
		                    </div>
		                    <div class="col-md-7 text-right">
		                        halaman : <?php echo htmlSelectFromArray($paging, 'name="p" style="width:100px" onchange="window.location=\'' . base_url('ad_tag_view/action/view/') . $f . '/' . $l . '/\'+this.value"', false, $p); ?>
		                    </div>
		        ...
		        <div class="modal-dialog fade-left">
		            <div class="modal-content">
		                ...
		                <div class="modal-body">
		                    <form action="<?php echo base_url('ad_tag_view/action/save') ?>" method="post" id="modForm" class="form-horizontal" enctype="multipart/form-data">
		...
		<script>
			if (!checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ad_tag_view');?>')) {
		        history.go(-1);
		    }
		    ...
		    $(document).ready(function () {
		        ...
		        $('.tr_mod').click(function () {
		            if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>','<?php echo aksesName('ad_tag_edit');?>')) {
		                var tr = $(this).parent();
		    ...
		    function formAdd() {
		        if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ad_tag_add');?>')) {
		            $('#modalMod :input').val('');
		    ...
		    function formDelete() {
		        if (checkPrivilege('<?php echo $this->session->userdata('id_level');?>', '<?php echo aksesName('ad_tag_delete');?>')) {
		            if (confirm('Are you sure want to delete this data?')) {
		                ...
		                window.location = '<?php echo base_url('ad_tag_view/action/delete/')?>' + (idC.join('k'));
		            }
		        }
		    }
		</script>