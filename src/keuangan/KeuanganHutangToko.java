...
public final class KeuanganHutangToko extends javax.swing.JDialog {
    ...
    private int row=0,i;
    private String koderekening="",tanggaldatang="",tanggaltempo="",akunbayar=Sequel.cariIsi("select Bayar_Pemesanan_Toko from set_akun");
        ...
        if(tabMode.getRowCount()==0){
            ...
            for(int i=0;i<row;i++){  
                    Sequel.menyimpan("temporary","'0','"+
                                ...
                                tabMode.getValueAt(i,13).toString()+"','','','','','','','','','','','','','','','','','','','','','','','','','',''","Hutang"); 
            }
            ...
            Valid.MyReport("rptHutangTokoBelumLunas.jasper","report","::[ Data Hutang Barang Toko / Minimarket / Koperasi ]::",param);
        }
                ...
                if(tabMode.getValueAt(i,0).toString().equals("true")){
                    if(Double.parseDouble(tbBangsal.getValueAt(i,10).toString())>0){
                        if(Sequel.menyimpantf("toko_bayar_pemesanan","?,?,?,?,?,?,?","Data", 7,new String[]{
                            ...
                            jur.simpanJurnal(no_bukti.getText(),Valid.SetTgl(tgl_bayar.getSelectedItem()+""),"U","BAYAR PELUNASAN HUTANG TOKO NO.FAKTUR "+tabMode.getValueAt(i,1).toString()+", OLEH "+akses.getkode());                            
                        }
                    }                        
                }
            }
