...
public class DlgBilingRanap extends javax.swing.JDialog {
    ...
    private String umur = "", biaya = "", tambahan = "", totals = "", norawatbayi = "", centangdokterranap = "", kd_pj = "",
            ...
            + "resep_pulang.jml_barang,resep_pulang.aturan_pakai,resep_pulang.total "
            ...
            public void windowClosed(WindowEvent e) {
                if (akses.getform().equals("DLgBilingRanap")) {
                    if (status.equals("belum")) {
                        ...
                        uangdeposit = uangMuka + uangLunasObat;
                    } else {
//                        uangdeposit=Sequel.cariIsiAngka("select ifnull(sum(Uang_Muka),0) from nota_inap where no_rawat=?",TNoRw.getText());
                    ...
                    Deposit.setText(Valid.SetAngka(uangMuka + uangLunasObat));
                    isKembali();
                }
            }
    ...
    private void initComponents() {
        ...
        setMinimumSize(new java.awt.Dimension(924, 204));
        setUndecorated(true);
        setResizable(false);
        ...
        DTPTgl.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "17-11-2020 09:12:54" }));
    ...
    private void BtnKeluarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKeluarActionPerformed
        if (akses.getbilling_ranap() == true) {
            try {
                ...
                if (i <= 0) {
                    int jawab = JOptionPane.showConfirmDialog(rootPane, "Data pembayaran belum tersimpan, apa anda mau menyimpannya...????", "Konfirmasi", JOptionPane.YES_NO_OPTION);
    ...
    private void BtnViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnViewActionPerformed
        ...
        int pilih = 0;
        try {
            input = (String) JOptionPane.showInputDialog(rootPane, "Silahkan pilih yang mau ditampilkan!", "Keuangan", JOptionPane.QUESTION_MESSAGE, null, options, "Nota 1");
            switch (input) {
                case "Tagihan Masuk":
                    pilih = 1;
                    break;
...
private void MnHapusTagihanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnHapusTagihanActionPerformed
    try {
        ...
        if (i > 0) {
            ...
            if (piutang > 0) {
                jur.simpanJurnal(TNoRw.getText(), Valid.SetDateToString(DTPTgl.getDate()), "U", "PEMBATALAN PIUTANG PASIEN RAWAT INAP, DIBATALKAN OLEH " + akses.getkode());
            } else if (piutang <= 0) {
                jur.simpanJurnal(TNoRw.getText(), Valid.SetDateToString(DTPTgl.getDate()), "U", "PEMBATALAN PEMBAYARAN PASIEN RAWAT INAP, DIBATALKAN OLEH " + akses.getkode());
            }
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if (TNoRw.getText().trim().equals("") || TNoRM.getText().trim().equals("") || TPasien.getText().trim().equals("")) {
            ...
            JOptionPane.showMessageDialog(rootPane, "Maaf, data tagihan pasien dengan No.Rawat tersebut sudah pernah disimpan...!!!");
        } else if (i == 0) {
            if (piutang <= 0) {
                if (kekurangan < 0) {
                    JOptionPane.showMessageDialog(rootPane, "Maaf, pembayaran pasien masih kurang ...!!!");
                } else if (kekurangan > 0) {
                    if (countbayar > 1) {
                        JOptionPane.showMessageDialog(rootPane, "Maaf, kembali harus bernilai 0 untuk cara bayar lebih dari 1...!!!");
                    } else {
                        if (ChkPiutang.isSelected() == true) {
                            JOptionPane.showMessageDialog(rootPane, "Maaf, matikan centang di piutang ...!!!");
                        } else {
                            isSimpan();
                        }
                    }
                ...
                if (ChkPiutang.isSelected() == true) {
                    if (kekurangan < 0) {
                        JOptionPane.showMessageDialog(rootPane, "Maaf, piutang belum genap. Silahkan isi di jumlah piutang ...!!!");
                    } else if (kekurangan > 0) {
                        JOptionPane.showMessageDialog(rootPane, "Maaf, terjadi kelebihan piutang ...!!!");
                    } else {
                        isSimpan();
                    }
                    ...
                    if (i == 1) {
                        ...
                        if (piutang > 0) {
                            Valid.panggilUrl("billing/LaporanBilling4.php?petugas=" + akses.getkode().replaceAll(" ", "_") + "&nonota=" + Sequel.cariIsi("select count(kamar_inap.no_rawat) from kamar_inap inner join reg_periksa "
                                    + "on kamar_inap.no_rawat=reg_periksa.no_rawat where reg_periksa.kd_pj='" + kd_pj + "' and kamar_inap.tgl_keluar like '%" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "%'") + "/RI/"
                                    + kd_pj + "/" + Valid.SetDateToString(DTPTgl.getDate()).substring(5, 7) + "/" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 4) + "&no_rawat=" + TNoRw.getText() + "&pasien=" + TPasien.getText().replaceAll(" ", "_") + "&bayar=" + bayar);
                        } else if (piutang <= 0) {
                            Valid.panggilUrl("billing/LaporanBilling4.php?petugas=" + akses.getkode().replaceAll(" ", "_") + "&nonota=" + Sequel.cariIsi("select count(kamar_inap.no_rawat) from kamar_inap inner join reg_periksa "
                                    + "on kamar_inap.no_rawat=reg_periksa.no_rawat where reg_periksa.kd_pj='" + kd_pj + "' and kamar_inap.tgl_keluar like '%" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "%'") + "/RI/"
                                    + kd_pj + "/" + Valid.SetDateToString(DTPTgl.getDate()).substring(5, 7) + "/" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 4) + "&no_rawat=" + TNoRw.getText() + "&pasien=" + TPasien.getText().replaceAll(" ", "_") + "&bayar=" + bayar);
    ...
    private void prosesCariReg() {
        ...
        try {
            ...
            try {
                ...
                while (rsreg.next()) {
                    noNota = Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_nota,6),signed)),0) from nota_inap where left(tanggal,7)='" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "' ", Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7).replaceAll("-", "/") + "/RI/", 6);

                    tabModeRwJlDr.addRow(new Object[]{true, "No.Nota", ": " + Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_nota,6),signed)),0) from nota_inap where left(tanggal,7)='" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "' ", Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7).replaceAll("-", "/") + "/RI/", 6), "", null, null, null, null, "-"});
        ...
        try {
            ...
            try {
                ...
                while (rsreseppulang.next()) {
                    ...
                    try {
                        pstamkur.setString(1, TNoRw.getText());
                        pstamkur.setString(2, rsreseppulang.getString("nama_brng") + " " + rsreseppulang.getString("aturan_pakai"));
                    ...
                    tabModeRwJlDr.addRow(new Object[]{true, "                           ", rsreseppulang.getString("nama_brng") + " " + rsreseppulang.getString("aturan_pakai"), ":",
                        ...
                        if (chkRalan.isSelected() == true) {
                            ...
                            while (rsralandrpr.next()) {
                                ...
                                if (rinciandokterranap.equals("Yes")) {
                                    ...
                                    String namaTindakan = rsralandrpr.getString("nm_perawatan").contains("Periksa")
        ...
        try {
            try {
                ...
                try {
                    ...
                    psnota.setString(2, Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_nota,6),signed)),0) from nota_inap where left(tanggal,7)='" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "' ", Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7).replaceAll("-", "/") + "/RI/", 6));
                    ...
                    Sequel.meghapus("nota_inap", "no_rawat", TNoRw.getText());
                    tbBilling.setValueAt(": " + Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_nota,6),signed)),0) from nota_inap where left(tanggal,7)='" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "' ", Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7).replaceAll("-", "/") + "/RI/", 6), 0, 2);
                    psnota = koneksi.prepareStatement(sqlpsnota);
                    try {
                        psnota.setString(1, TNoRw.getText());
                        psnota.setString(2, Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_nota,6),signed)),0) from nota_inap where left(tanggal,7)='" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7) + "' ", Valid.SetDateToString(DTPTgl.getDate()).substring(0, 7).replaceAll("-", "/") + "/RI/", 6));
            ...
            if (piutang > 0) {
                jur.simpanJurnal(TNoRw.getText(), Valid.SetDateToString(DTPTgl.getDate()), "U", "PIUTANG PASIEN RAWAT INAP, DIPOSTING OLEH " + akses.getkode());
                ...
                Sequel.queryu2("insert into piutang_pasien values ('" + TNoRw.getText() + "','" + Valid.SetDateToString(DTPTgl.getDate()) + "','"
                        + TNoRM.getText() + "','Belum Lunas','" + total + "','" + (bayar + uangdeposit) + "','" + piutang + "','" + Valid.SetDateToString(DTPTgl.getDate()) + "')");
            } else if (piutang <= 0) {
                jur.simpanJurnal(TNoRw.getText(), Valid.SetDateToString(DTPTgl.getDate()), "U", "PEMBAYARAN PASIEN RAWAT INAP, DIPOSTING OLEH " + akses.getkode());
                Sequel.menyimpan2("tagihan_sadewa", "'" + TNoRw.getText() + "','" + TNoRM.getText() + "','" + TPasien.getText() + "','" + alamat + "',concat('" + Valid.SetDateToString(DTPTgl.getDate())
