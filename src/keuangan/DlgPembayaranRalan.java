...
public final class DlgPembayaranRalan extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Record.setForeground(new java.awt.Color(50, 50, 50));
        Record.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        Record.setText("0");
        Record.setName("Record"); // NOI18N
        Record.setPreferredSize(new java.awt.Dimension(80, 23));
        panelGlass5.add(Record);
        ...
        cmbStatus.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Semua", "Belum Bayar", "Sudah Bayar" }));
...
private void BtnCari1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCari1KeyPressed
    ...
    public void tampil() {
        ...
        try {
            if (NmPoli.getText().equals("") && NmCaraBayar.getText().equals("")
                    && NmDokter.getText().equals("") && cmbStatus.getSelectedItem().toString().equalsIgnoreCase("Semua")) {
                ps = koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,reg_periksa.tgl_registrasi,dokter.nm_dokter,poliklinik.nm_poli,reg_periksa.status_bayar "
                ...
                if (cmbStatus.getSelectedItem().equals("Semua")) {
                    ps = koneksi.prepareStatement(
                            ...
                            + "and reg_periksa.no_rawat not in (select piutang_pasien.no_rawat from piutang_pasien where piutang_pasien.no_rawat=reg_periksa.no_rawat) and "
                ...
                } else if (cmbStatus.getSelectedItem().equals("Belum Bayar")) {
                    ps = koneksi.prepareStatement(
                            "select reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,reg_periksa.tgl_registrasi,dokter.nm_dokter,poliklinik.nm_poli,reg_periksa.status_bayar "
                            + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                            + "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli "
                            ...
                            + "and concat(reg_periksa.kd_dokter,dokter.nm_dokter) like ? and concat(reg_periksa.kd_pj,penjab.png_jawab) like ? "
                            + "order by reg_periksa.kd_dokter,reg_periksa.tgl_registrasi");
                } else if (cmbStatus.getSelectedItem().equals("Sudah Bayar")) {
                    ps = koneksi.prepareStatement("select reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,reg_periksa.tgl_registrasi,dokter.nm_dokter,poliklinik.nm_poli,reg_periksa.status_bayar "
                            + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                            + "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli "
                            + "inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter "
                            + "inner join penjab on reg_periksa.kd_pj=penjab.kd_pj "
                            + "left join piutang_pasien on reg_periksa.no_rawat=piutang_pasien.no_rawat "
                            + "where "
                            + "reg_periksa.status_bayar='Sudah Bayar' "
            ...
            try {
                if (NmPoli.getText().equals("") && NmCaraBayar.getText().equals("") && NmDokter.getText().equals("") && cmbStatus.getSelectedItem().toString().equalsIgnoreCase("Semua")) {
...
//                    }
                    ...
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("nm_dokter"), rs.getString("reg_periksa.status_bayar")
                    });
                }
                tabMode.addRow(new Object[]{
                    ">> Total", ":", "", "", "",
