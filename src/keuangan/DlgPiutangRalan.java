...
public final class DlgPiutangRalan extends javax.swing.JDialog {
    ...
    public DlgPiutangRalan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] rowRwJlDr = {"Tanggal", "No.Nota", "No. SEP", "NRM", "Nama Pasien", "Jenis Bayar", "No Peserta",
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            for (int r = 0; r < tabMode.getRowCount(); r++) {
                Sequel.menyimpan("temporary", "'0','"
                        ...
                        + tabMode.getValueAt(r, 19).toString().replaceAll("'", "`") + "','','','','','','','','','','','','','','','','','','',''", "data");
            }
            ...
            Valid.MyReport("rptRPiutangRalan.jasper", "report", "::[ Rekap Piutang Ralan Masuk ]::", param);
        }
    ...
    public void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + "where "
                    + "reg_periksa.status_lanjut='Ralan' and reg_periksa.tgl_registrasi between ? and ? and concat(reg_periksa.kd_pj,penjab.png_jawab) like ? and reg_periksa.no_rawat like ? or "
                    + "reg_periksa.status_lanjut='Ralan' and reg_periksa.tgl_registrasi between ? and ? and concat(reg_periksa.kd_pj,penjab.png_jawab) like ? and reg_periksa.no_rkm_medis like ? or "
                    + "reg_periksa.status_lanjut='Ralan' and reg_periksa.tgl_registrasi between ? and ? and concat(reg_periksa.kd_pj,penjab.png_jawab) like ? and pasien.nm_pasien like ? or "
                    + "reg_periksa.status_lanjut='Ralan' and reg_periksa.tgl_registrasi between ? and ? and concat(reg_periksa.kd_pj,penjab.png_jawab) like ? and bridging_sep.no_sep like ? "
                    + "order by reg_periksa.tgl_registrasi asc,reg_periksa.kd_dokter");
            try {
                ...
                while (rs.next()) {
                    ...
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("no_peserta"), Valid.SetAngka(Registrasi),
                        Valid.SetAngka(Obat), Valid.SetAngka(Ralan_Dokter + Ralan_Paramedis + Ralan_Dokter_paramedis),
                        Valid.SetAngka(Operasi), Valid.SetAngka(Laborat),
                        Valid.SetAngka(Radiologi), Valid.SetAngka(Tambahan),
                        Valid.SetAngka(Potongan), Valid.SetAngka(Operasi + Laborat + Radiologi + Obat + Ralan_Dokter + Ralan_Paramedis + Ralan_Dokter_paramedis + Tambahan + Potongan + Registrasi),
                        Valid.SetAngka(ekses), Valid.SetAngka(dibayar),
                        Valid.SetAngka(sisa), rs.getString("nm_dokter")
                    });
                }
                if (tabMode.getRowCount() > 0) {
                    tabMode.addRow(new Object[]{
