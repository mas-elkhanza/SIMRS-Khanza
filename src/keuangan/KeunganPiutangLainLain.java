 ...
 * @author dosen
 */
public final class KeunganPiutangLainLain extends javax.swing.JDialog {
    ...
    private String koderekening="",kontraakun="";
     ...
     * @param parent
     * @param modal */
    public KeunganPiutangLainLain(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        Keterangan.setBounds(399, 70, 321, 23);
        ...
        NmPeminjam.setBounds(461, 10, 228, 23);
        ...
        BtnPeminjam.setBounds(692, 10, 28, 23);
        ...
        NmPetugas.setBounds(491, 40, 198, 23);
        ...
        BtnPetugas.setBounds(692, 40, 28, 23);
        ...
        Tanggal1.setBounds(210, 40, 90, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        /*if(NoNota.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("bayar_piutang","?,?,?,?,?,?,?","Pembayaran",7,new String[]{
                    ...
                    Sequel.mengedit("detail_piutang_pasien","no_rawat='"+NoNota.getText()+"' and nama_bayar='"+AkunPiutang.getSelectedItem().toString()+"'","sisapiutang=sisapiutang-"+Cicilan.getText());
                    ...
                    sukses=jur.simpanJurnal(NoNota.getText(),"U","BAYAR PIUTANG"+", OLEH "+akses.getkode());                   
            }else{
                sukses=false;
            }  
        ...
        /*if(NoNota.getText().trim().equals("")){
            ...
            if(tbKamar.getSelectedRow()>-1){
                ...
                if(Sequel.queryu2tf("delete from bayar_piutang where tgl_bayar=? and no_rkm_medis=? and no_rawat=? and kd_rek=? and kd_rek_kontra=?", 5,new String[]{
                    ...
                    sukses=jur.simpanJurnal(NoNota.getText(),"U","PEMBATALAN BAYAR PIUTANG"+", OLEH "+akses.getkode());     
                }else{
                    sukses=false;
                }
   ...
   //Valid.pindah(evt,Cicilan,KdPeminjam);
    ...
    private void KdPeminjamKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KdPeminjamKeyPressed
        /*if(evt.getKeyCode()==KeyEvent.VK_PAGE_DOWN){
            Sequel.cariIsi("select nm_pasien from pasien where no_rkm_medis=?", NmPeminjam,KdPeminjam.getText());
            sisapiutang=Sequel.cariIsiAngka("SELECT ifnull(SUM(piutang_pasien.sisapiutang),0) FROM piutang_pasien where piutang_pasien.no_rkm_medis=?",KdPeminjam.getText())
                         +
                        Sequel.cariIsiAngka("SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.no_rkm_medis=?",KdPeminjam.getText())
                        - 
                        Sequel.cariIsiAngka("SELECT ifnull(SUM(bayar_piutang.besar_cicilan),0) FROM bayar_piutang where bayar_piutang.no_rkm_medis=?",KdPeminjam.getText());
            ...
            Sequel.cariIsi("select no_rawat from reg_periksa where no_rkm_medis=? order by tgl_registrasi desc limit 1", NoNota,KdPeminjam.getText());            
        }else if(evt.getKeyCode()==KeyEvent.VK_PAGE_UP){
            Sequel.cariIsi("select nm_pasien from pasien where no_rkm_medis=?", NmPeminjam,KdPeminjam.getText());
            sisapiutang=Sequel.cariIsiAngka("SELECT ifnull(SUM(piutang_pasien.sisapiutang),0) FROM piutang_pasien where piutang_pasien.no_rkm_medis=?",KdPeminjam.getText())
                         +
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeunganPiutangLainLain dialog = new KeunganPiutangLainLain(new javax.swing.JFrame(), true);
    ...
    public void setData(String norawat,String norm,String nama){
        ...
        KdPeminjam.setText(norm);
        NmPeminjam.setText(nama);
        TCari.setText(norawat);
        ChkInput.setSelected(true);
        isForm();
        kontraakun="";
        if(Sequel.cariInteger("select count(nota_piutang) from piutang where nota_piutang=?",NoNota.getText())==1){
            ...
            sisapiutang=Sequel.cariIsiAngka("SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.nota_piutang=?",NoNota.getText())
                        -Sequel.cariIsiAngka("SELECT ifnull(SUM(bayar_piutang.besar_cicilan),0) FROM bayar_piutang where bayar_piutang.no_rawat=?",NoNota.getText());
            Sisa.setText(Valid.SetAngka(Valid.roundUp(sisapiutang,100)));
            status="obat";
        }else{
            Valid.loadCombo(AkunPiutang,"select nama_bayar from detail_piutang_pasien where no_rawat='"+NoNota.getText()+"'");
            kontraakun=Sequel.cariIsi("select kd_rek from akun_piutang where nama_bayar=?",AkunPiutang.getSelectedItem().toString());
            sisapiutang=Sequel.cariIsiAngka("select sisapiutang from detail_piutang_pasien where no_rawat='"+NoNota.getText()+"' and nama_bayar='"+AkunPiutang.getSelectedItem().toString()+"'");
            Sisa.setText(Valid.SetAngka(Valid.roundUp(sisapiutang,100)));
            status="pasien";
        }*/
    }
