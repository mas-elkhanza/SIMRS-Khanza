 ...
 * @author RSUI HA
 */
public class DlgBayarPinjamObat extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public DlgBayarPinjamObat(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Pengembalian Pinjam Obat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("sansserif", 1, 12), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        panelisi3.add(kdptg);
        kdptg.setBounds(410, 10, 80, 23);

        nmptg.setEditable(false);
        nmptg.setName("nmptg"); // NOI18N
        nmptg.setPreferredSize(new java.awt.Dimension(207, 23));
        panelisi3.add(nmptg);
        nmptg.setBounds(490, 10, 240, 23);
        ...
        btnPetugas.setBounds(740, 10, 28, 23);

        label20.setText("Lokasi :");
        label20.setName("label20"); // NOI18N
        label20.setPreferredSize(new java.awt.Dimension(70, 23));
        panelisi3.add(label20);
        label20.setBounds(0, 70, 70, 23);
        ...
        nmgudang.setBounds(160, 70, 150, 23);
        ...
        btnGudang.setBounds(310, 70, 28, 23);

        label14.setText("Petugas :");
        label14.setName("label14"); // NOI18N
        label14.setPreferredSize(new java.awt.Dimension(70, 23));
        panelisi3.add(label14);
        label14.setBounds(340, 10, 70, 23);

        label16.setText("Keterangan :");
        label16.setName("label16"); // NOI18N
        label16.setPreferredSize(new java.awt.Dimension(70, 23));
        panelisi3.add(label16);
        label16.setBounds(330, 40, 80, 23);
        ...
        keterangan.setBounds(410, 40, 320, 23);
                ...
                for (i = 0; i < jml; i++) {
                    ...
                    try {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("bayar_pinjam_obat", "?,?,?,?,?,?,?", "No.Faktur", 7, new String[]{
                                NoFaktur.getText(), tbDokter.getValueAt(i, 2).toString(),
                                tbDokter.getValueAt(i, 1).toString(), tbDokter.getValueAt(i, 0).toString(), keterangan.getText(),
                                Valid.SetTgl(TglPesan.getSelectedItem() + ""), kdptg.getText()
                            }) == true) {
                                if (aktifkanbatch.equals("yes")) {
                                    Trackobat.catatRiwayat(tbDokter.getValueAt(i, 2).toString(), 0, Valid.SetAngka(tbDokter.getValueAt(i, 12).toString()), "Pengembalian Obat", akses.getkode(), kdgudang.getText(), "Simpan", tbDokter.getValueAt(i, 13).toString(), NoFaktur.getText());
                                    ...
                                    Trackobat.catatRiwayat(tbDokter.getValueAt(i, 2).toString(), 0, Valid.SetAngka(tbDokter.getValueAt(i, 12).toString()), "Pengembalian Obat", akses.getkode(), kdgudang.getText(), "Simpan", "", "");
...
private void btnGudangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGudangActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgBayarPinjamObat dialog = new DlgBayarPinjamObat(new javax.swing.JFrame(), true);
...
//        Meterai.setText("0");
    ...
    public void isCek(String Faktur) {
        try {
            ...
            ps = koneksi.prepareStatement("select detailpinjamobat.kode_brng,databarang.nama_brng,ifnull(date_format(databarang.expire,'%d-%m-%Y'),'00-00-0000') as expire, "
                    + "detailpinjamobat.kode_sat,kodesatuan.satuan,detailpinjamobat.jumlah,detailpinjamobat.h_pesan,databarang.isi, "
                    + "detailpinjamobat.subtotal,detailpinjamobat.dis,detailpinjamobat.besardis,detailpinjamobat.total,detailpinjamobat.no_batch, "
                    + "databarang.kode_sat as satbar,detailpinjamobat.jumlah2 from detailpinjamobat inner join databarang inner join kodesatuan inner join jenis "
                    + " on detailpinjamobat.kode_brng=databarang.kode_brng and databarang.kdjns=jenis.kdjns "
                    + " and detailpinjamobat.kode_sat=kodesatuan.kode_sat where detailpinjamobat.no_faktur=? ");
            try {
    ...
    public void tampil(String noorder) {
        NoFaktur.setText(noorder);
        meterai = Sequel.cariIsiAngka("select meterai from surat_pinjam_obat_medis where no_pinjam_obat=?", noorder);
        ppn = Sequel.cariIsiAngka("select ppn from surat_pinjam_obat_medis where no_pinjam_obat=?", noorder);
        try {
            Valid.tabelKosong(tabMode);
            ps = koneksi.prepareStatement(
                    "select detail_surat_pinjam_obat_medis.kode_brng, databarang.nama_brng,"
                    + "detail_surat_pinjam_obat_medis.kode_sat,detail_surat_pinjam_obat_medis.h_pesan, "
                    + "ifnull(date_format(databarang.expire,'%d-%m-%Y'),'00-00-0000'),"
                    + "detail_surat_pinjam_obat_medis.jumlah,detail_surat_pinjam_obat_medis.subtotal,"
                    + "detail_surat_pinjam_obat_medis.dis,detail_surat_pinjam_obat_medis.besardis,"
                    + "detail_surat_pinjam_obat_medis.total,detail_surat_pinjam_obat_medis.jumlah2,databarang.kode_sat as satbar, "
                    + "databarang.isi from databarang inner join jenis inner join detail_surat_pinjam_obat_medis "
                    + "on databarang.kdjns=jenis.kdjns and detail_surat_pinjam_obat_medis.kode_brng=databarang.kode_brng "
                    + " where detail_surat_pinjam_obat_medis.no_pinjam_obat=? order by databarang.nama_brng");
            try {
    ...
    private void simpanbatch() {
        if ((!tbDokter.getValueAt(i, 13).toString().equals("")) && (!tbDokter.getValueAt(i, 14).toString().equals(""))
            ...
            Sequel.menyimpan2("data_batch", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Batch", 20, new String[]{
                tbDokter.getValueAt(i, 13).toString(), tbDokter.getValueAt(i, 2).toString(), Valid.SetTgl(TglPesan.getSelectedItem() + ""), Valid.SetTgl(tbDokter.getValueAt(i, 6).toString()), "Peminjaman", NoFaktur.getText(),
