...
public class DlgPengeluaranApotek extends javax.swing.JDialog {
            ...
            public void windowClosed(WindowEvent e) {
                if (akses.getform().equals("DlgPengeluaranApotek")) {
                    if (form.bangsal.getTable().getSelectedRow() != -1) {
                        kdgudang.setText(form.bangsal.getTable().getValueAt(form.bangsal.getTable().getSelectedRow(), 0)
                                .toString());
                        nmgudang.setText(form.bangsal.getTable().getValueAt(form.bangsal.getTable().getSelectedRow(), 1)
                                .toString());
                        tampil();
                    }
    ...
    private void initComponents() {
        ...
        tbDokter.setModel(new javax.swing.table.DefaultTableModel(new Object[][] { {}, {}, {}, {} }, new String[] {
                        ...
                        for (i = 0; i < jml; i++) {
                            if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                                if (Sequel.menyimpantf2("detail_pengeluaran_obat_bhp", "?,?,?,?,?,?,?,?",
                                        ...
                                        new String[] { NoKeluar.getText(), tbDokter.getValueAt(i, 1).toString(),
                                                ...
                                                tbDokter.getValueAt(i, 9).toString() }) == true) {
                                    if (aktifkanbatch.equals("yes")) {
                            ...
                            if (ttl > 0) {
                                ...
                                Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4,
                                        new String[] { Sequel.cariIsi("select Kontra_Stok_Keluar_Medis from set_akun"),
                                                "KONTRA PERSEDIAAN BARANG", "" + (ttl), "0" });
                                sukses = jur.simpanJurnal(NoKeluar.getText(), Valid.SetTgl(Tgl.getSelectedItem() + ""),
                                        "U", "STOK KELUAR BARANG MEDIS/OBAT/ALKES/BHP" + ", OLEH " + akses.getkode());
                            }
                        }
                    } catch (Exception ex) {
                        sukses = false;
        ...
        for (i = 0; i < tbDokter.getRowCount(); i++) {
            try {
                ...
                psstok = koneksi.prepareStatement(
                        "select ifnull(stok,'0') from gudangbarang where kd_bangsal=? and kode_brng=? and no_batch=? and no_faktur=?");
                try {
                ...
                try {
                    ...
                    while (rs.next()) {
                        tabMode.addRow(new Object[] { "", rs.getString("kode_brng"), "", rs.getString("nama_brng"),
                                rs.getString("nama"), rs.getString("kode_sat"), rs.getDouble("h_beli"), 0,
                                rs.getDouble("stok"), "" });
                    }
                } catch (Exception e) {
                    System.out.println("Notifikasi : " + e);
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(no_keluar,3),signed)),0) from pengeluaran_obat_bhp where tanggal='"
                        + Valid.SetTgl(Tgl.getSelectedItem() + "") + "' ",
                "SKM" + Tgl.getSelectedItem().toString().substring(8, 10)
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[] { rs.getString("jumlah"), rs.getString("kode_brng"), "",
                            rs.getString("nama_brng"), rs.getString("nama"), rs.getString("kode_sat"),
                            rs.getDouble("h_beli"), (rs.getDouble("jumlah") * rs.getDouble("h_beli")), 0, "" });
                }
            } catch (Exception e) {
                System.out.println("Note : " + e);
            } finally {
                    ...
                    if (aktifkanbatch.equals("yes")) {
                        ...
                        try {
                            ...
                            if (rsstok.next()) {
                                ...
                                tbDokter.setValueAt((rsstok.getDouble("h_beli")
