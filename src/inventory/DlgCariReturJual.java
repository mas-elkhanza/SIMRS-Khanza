...
public class DlgCariReturJual extends javax.swing.JDialog {
    ...
    public DlgCariReturJual(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[] { "No.Retur", "Tgl.Retur", "Petugas", "Pasien", "No.Nota",
                "Barang", "Satuan", "Harga Retur(Rp)", "Jml", "SubTotal(Rp)", "nip" }) {
            @Override
            ...
            public void windowClosed(WindowEvent e) {
                if (akses.getform().equals("DlgCariReturJual")) {
                    if (barang.satuan.getTable().getSelectedRow() != -1) {
                        kdsat.setText(barang.satuan.getTable().getValueAt(barang.satuan.getTable().getSelectedRow(), 0)
                                .toString());
                        nmsat.setText(barang.satuan.getTable().getValueAt(barang.satuan.getTable().getSelectedRow(), 1)
                                .toString());
                    }
                    kdsat.requestFocus();
                }
    ...
    private void initComponents() {
        ...
        notaRetur = new javax.swing.JMenuItem();
        ...
        notaRetur.setBackground(new java.awt.Color(255, 255, 254));
        notaRetur.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        notaRetur.setForeground(new java.awt.Color(50, 50, 50));
        notaRetur.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        notaRetur.setText("Nota Retur");
        notaRetur.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        notaRetur.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        notaRetur.setName("notaRetur"); // NOI18N
        notaRetur.setPreferredSize(new java.awt.Dimension(150, 25));
        notaRetur.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                notaReturActionPerformed(evt);
            }
        });
        jPopupMenu1.add(notaRetur);
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Cari Retur Obat, Alkes & BHP Medis Dari Pembeli/Pasien ]::",
        ...
        tbRetur.setModel(new javax.swing.table.DefaultTableModel(new Object[][] { {}, {}, {}, {} }, new String[] {
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            for (int i = 0; i < row; i++) {
                Sequel.menyimpan("temporary", "'0','" + tabMode.getValueAt(i, 0).toString() + "','"
                        ...
                        + tabMode.getValueAt(i, 3).toString() + "','" + tabMode.getValueAt(i, 4).toString() + "','"
            ...
            Valid.MyReport("rptReturJual.jasper", "report", "::[ Transaksi Retur Jual ]::", param);
        }
        this.setCursor(Cursor.getDefaultCursor());
                    ...
                    if (rs.next()) {
                            ...
                            while (rs2.next()) {
                                if (aktifkanbatch.equals("yes")) {
                                    Sequel.mengedit("data_batch", "no_batch=? and kode_brng=? and no_faktur=?",
                                            "sisa=sisa-?", 4,
                                            new String[] { rs2.getString("jml_retur"), rs2.getString("no_batch"),
                                                    rs2.getString("kode_brng"), rs2.getString("no_faktur") });
                                    Trackobat.catatRiwayat(rs2.getString("kode_brng"), 0, rs2.getDouble("jml_retur"),
                                            "Retur Jual", akses.getkode(), rs.getString("kd_bangsal"), "Hapus",
                                            rs2.getString("no_batch"), rs2.getString("no_faktur"));
                                    Sequel.menyimpan("gudangbarang", "'" + rs2.getString("kode_brng") + "','"
                                            + rs.getString("kd_bangsal") + "','-" + rs2.getString("jml_retur") + "','"
                                            + rs2.getString("no_batch") + "','" + rs2.getString("no_faktur") + "'",
                                            "stok=stok-'" + rs2.getString("jml_retur") + "'",
                        ...
                        if (sukses = true) {
                            Sequel.menyimpan("tampjurnal",
                                    "'" + Sequel.cariIsi("select Retur_Dari_pembeli from set_akun")
                                            + "','RETUR PENJUALAN','0','"
                                            + Sequel.cariIsi(
                                                    "select sum(subtotal) from detreturjual where no_retur_jual='"
                                                            + rs.getString("no_retur_jual") + "'")
                                            + "'",
                                    "Rekening");
                            Sequel.menyimpan("tampjurnal",
                                    "'" + Sequel.cariIsi("select Kontra_Retur_Dari_Pembeli from set_akun")
                                            + "','KAS DI TANGAN','"
                                            + Sequel.cariIsi(
                                                    "select sum(subtotal) from detreturjual where no_retur_jual='"
                                                            + rs.getString("no_retur_jual") + "'")
                                            + "','0'",
                                    "Rekening");
                            sukses = jur.simpanJurnal(rs.getString(1), Sequel.cariIsi("select current_date()"), "U",
                                    "BATAL RETUR PENJUALAN DI "
                                            + Sequel.cariIsi("select nm_bangsal from bangsal where kd_bangsal='"
                                                    + rs.getString("kd_bangsal") + "'").toUpperCase()
                                            + ", OLEH " + akses.getkode());
                        }

                        if (sukses == true) {
                            Sequel.queryu("delete from returjual where no_retur_jual='"
                                    + tbRetur.getValueAt(tbRetur.getSelectedRow(), 0).toString() + "'");
                            Sequel.Commit();
                        } else {
                            sukses = false;
    ...
    private void tbReturMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tbReturMouseClicked
        if (tabMode.getRowCount() != 0) {
            try {
                ...
                // String petugas =
                // StringUtils.substringAfter(tbRetur.getValueAt(tbRetur.getSelectedRow(),
                // 2).toString(), ", ");
                // String kdpetugas =
                // StringUtils.substringBefore(tbRetur.getValueAt(tbRetur.getSelectedRow(),
                // 10).toString(), ", ");
    ...
    private javax.swing.JMenuItem notaRetur;
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                ttlretur = 0;
                while (rs.next()) {
                    tabMode.addRow(new Object[] { rs.getString(1), rs.getString(2), rs.getString(4), rs.getString(6),
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select detreturjual.nota_jual,detreturjual.kode_brng,databarang.nama_brng, "
                                    ...
                                    + " and detreturjual.nota_jual like '%" + TCari.getText() + "%' or "
                                    + " detreturjual.no_retur_jual='" + rs.getString(1) + "' " + sat + bar + nonot
                                    + " and returjual.no_rkm_medis like '%" + TCari.getText() + "%' or "
                                    + " detreturjual.no_retur_jual='" + rs.getString(1) + "' " + sat + bar + nonot
