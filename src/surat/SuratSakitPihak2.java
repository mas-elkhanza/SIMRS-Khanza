...
public final class SuratSakitPihak2 extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TanggalAkhir.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-08-2024" }));
        ...
        TglLahir.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-08-2024" }));
        ...
        TanggalAwal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-08-2024" }));
       ...
       if(TPasien.getText().trim().equals("")){
        ...
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                ...
                Valid.MyReportqry("rptSuratSakitPihakKe2.jasper","report","::[ Surat Sakit Pihak Ke 2 ]::",
                              ...
                              " DATE_FORMAT(reg_periksa.tgl_registrasi,'%d-%m-%Y')as tgl_registrasi,pasien.nm_pasien,pasien.jk,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur)as umurpasien,pasien.pekerjaan,concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) as alamat" +
    ...
    */
    ...
    public void emptTeks() {
        ...
        //Valid.autoNomer2("select ifnull(MAX(CONVERT(LEFT(suratsakitpihak2.no_surat,3),signed)),0) from suratsakitpihak2 where suratsakitpihak2.tanggalawal like '%" + Valid.SetTgl(TanggalAwal.getSelectedItem() + "").substring(0, 7) + "%' ",Valid.SetTgl(TanggalAwal.getSelectedItem() + "").substring(0, 4)+"/SKS-2/" + bln_romawi + "/", 3, NoSurat);
                Valid.autoNomer7("select ifnull(MAX(CONVERT(LEFT(no_surat,3),signed)),0),MONTH(tanggalawal) bln from suratsakitpihak2 where "
                + "tanggalawal like '%" + Valid.SetTgl(TanggalAwal.getSelectedItem()+ "").substring(0, 7) + "%' ", "/SKS2/RSUBM/" + bln_romawi + "/" + TanggalAwal.getSelectedItem().toString().substring(6,10), 3, NoSurat); 
                bln_angka = Sequel.cariIsi("SELECT MONTH(tanggalawal) bln FROM suratsakitpihak2 WHERE tanggalawal like '%" + Valid.SetTgl(TanggalAwal.getSelectedItem() + "").substring(0, 7) + "%'");
