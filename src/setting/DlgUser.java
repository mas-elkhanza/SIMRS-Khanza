...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[J]ICD 10","[J]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[M]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[L]Bridging TTE Satu Sehat","[L]Kirim Composition Satu Sehat","[K]Ringkasan Hutang Vendor Aset/Inventaris",
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                if(Sequel.mengedittf("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "ringkasan_hutang_vendor_inventaris='"+tbUser.getValueAt(i,1217).toString()+"'")==true){
                    emptTeks();
                }
            }         
        }
                ...
                new SwingWorker<Void, Object[]>() {
                    ...
                    protected Void doInBackground() throws Exception {
                        ps=koneksi.prepareStatement(
                            ...
                            "user.satu_sehat_mapping_kptl_tarif_kamar,user.checklist_kriteria_keluar_isolasi,user.satu_sehat_tanda_tangan_elektronik,user.satu_sehat_kirim_composition,user.ringkasan_hutang_vendor_inventaris from user order by AES_DECRYPT(user.id_user,'nur')");
                        try {
                            rs=ps.executeQuery();
                            i=0;
                            while(rs.next()){
