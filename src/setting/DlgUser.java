...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[J]ICD 10","[J]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[M]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[K]Piutang Jasa Perusahaan","[K]Bayar Piutang Jasa Perusahaan","[K]Piutang Jasa Perusahaan Belum Lunas","[M]Checklist Kesiapan Anestesi"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                if(Sequel.mengedittf("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "checklist_kesiapan_anestesi='"+tbUser.getValueAt(i,1131).toString()+"'")==true){
                    emptTeks();
                }
            }         
        }
                ...
                new SwingWorker<Void, Void>() {
                    ...
                    protected Void doInBackground() throws Exception {
                        ps=koneksi.prepareStatement(
                            ...
                            "user.kategori_piutang_jasa_perusahaan,user.piutang_jasa_perusahaan,user.bayar_piutang_jasa_perusahaan,user.piutang_jasa_perusahaan_belum_lunas,user.checklist_kesiapan_anestesi from user order by AES_DECRYPT(user.id_user,'nur')");
                        try {
                            rs=ps.executeQuery();
                            i=0;
                            while(rs.next()){
