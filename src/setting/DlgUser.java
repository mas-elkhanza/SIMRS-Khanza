...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[C]Audit Bundle IDO","[C]Audit Fasilitas Kebersihan Tangan","[C]Audit Fasilitas APD","[C]Audit Pembuangan Limbah Cair Infeksius"
        };
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.audit_fasilitas_kebersihan_tangan,user.audit_fasilitas_apd,user.audit_pembuangan_limbah_cair_infeksius from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
