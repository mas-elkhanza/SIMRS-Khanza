...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[C]Audit Penanganan Darah","[C]Audit Pengelolaan Linen Kotor","[C]Audit Penempatan Pasien","[C]Audit Kamar Jenazah"
        };
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.audit_pengelolaan_linen_kotor,user.audit_penempatan_pasien,user.audit_kamar_jenazah from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
