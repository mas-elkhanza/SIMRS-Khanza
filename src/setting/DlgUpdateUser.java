    ...
    */
    ...
    private void tampil(String user) {        
        try{    
            ...
            ps=koneksi.prepareStatement("select penyakit, obat_penyakit, dokter, jadwal_praktek, petugas, pasien, registrasi, tindakan_ralan,"+
                        ...
                        "permintaan_ranap,bpjs_diagnosa_prb,bpjs_obat_prb,bpjs_surat_kontrol,penggunaan_bhp_ok,surat_keterangan_rawat_inap from user where id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                while(rs.next()){
                    ...
                    if("[K]Surat Kontrol/SPRI VClaim".toLowerCase().contains(TCari.getText().toLowerCase())){
                        tabMode.addRow(new Object[]{false,"[K]Surat Kontrol/SPRI VClaim",rs.getBoolean("bpjs_surat_kontrol")});
                    }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[K]Surat Kontrol/SPRI VClaim".equals(tbUser.getValueAt(i,1).toString())){
