...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            rl1_3_ketersediaan_kamar=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.penilaian_bayi_baru_lahir,user.rl1_3_ketersediaan_kamar from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    hasil_pemeriksaan_echo=rs.getBoolean("hasil_pemeriksaan_echo");penilaian_bayi_baru_lahir=rs.getBoolean("penilaian_bayi_baru_lahir");rl1_3_ketersediaan_kamar=rs.getBoolean("rl1_3_ketersediaan_kamar");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[I]Pendapatan Per Akun".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[I]Pendapatan Per Akun",pendapatan_per_akun});
        }
    ...
    private void Simpan() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[I]Pendapatan Per Akun".equals(tbUser.getValueAt(i,1).toString())){
