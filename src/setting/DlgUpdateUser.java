...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            nilai_normal_baku_mutu_lab_kesehatan_lingkungan=false,skrining_pneumonia_severity_index=false,permintaan_pengujian_sampel_lab_kesehatan_lingkungan=false,rujuk_edukasi_berkelanjutan=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.permintaan_pengujian_sampel_lab_kesehatan_lingkungan from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
    ...
    private void setTampil(){
        ...
        if("[A]Rujuk Edukasi Berkelanjutan".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[A]Rujuk Edukasi Berkelanjutan",rujuk_edukasi_berkelanjutan});
        }
    ...
    private void Simpan() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[A]Rujuk Edukasi Berkelanjutan".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","rujuk_edukasi_berkelanjutan='"+tbUser.getValueAt(i,2).toString()+"'");
            }
