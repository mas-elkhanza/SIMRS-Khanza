    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.checklist_post_operasi,user.dapur_pembelian from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    dapur_barang=rs.getBoolean("dapur_barang");dapur_opname=rs.getBoolean("dapur_opname");satu_sehat_mapping_vaksin=rs.getBoolean("satu_sehat_mapping_vaksin");dapur_suplier=rs.getBoolean("dapur_suplier");satu_sehat_kirim_Immunization=rs.getBoolean("satu_sehat_kirim_Immunization");checklist_post_operasi=rs.getBoolean("checklist_post_operasi");dapur_pembelian=rs.getBoolean("dapur_pembelian");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[F]Suplier Dapur".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[E]Suplier Dapur",dapur_suplier});
        }
        
        if("[F]Pengadaan Barang Dapur".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[E]Pengadaan Barang Dapur",dapur_pembelian});
        }
    ...
    private void Simpan() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[E]Suplier Dapur".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","dapur_suplier='"+tbUser.getValueAt(i,2).toString()+"'");
            }
            
            if("[E]Pengadaan Barang Dapur".equals(tbUser.getValueAt(i,1).toString())){
