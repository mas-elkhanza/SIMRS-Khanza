...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            layanan_kedokteran_fisik_rehabilitasi=false,skrining_kesehatan_gigi_mulut_balita=false,skrining_anemia=false,layanan_program_kfr=false,sekrining_hipertensi=false,sekrining_kesehatan_penglihatan=false;
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.layanan_program_kfr,user.sekrining_hipertensi,user.sekrining_kesehatan_penglihatan from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    skrining_kesehatan_gigi_mulut_balita=rs.getBoolean("skrining_kesehatan_gigi_mulut_balita");skrining_anemia=rs.getBoolean("skrining_anemia");layanan_program_kfr=rs.getBoolean("layanan_program_kfr");sekrining_hipertensi=rs.getBoolean("sekrining_hipertensi");sekrining_kesehatan_penglihatan=rs.getBoolean("sekrining_kesehatan_penglihatan");
                    setTampil();
                }       
