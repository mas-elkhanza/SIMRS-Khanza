<?php
    ...
    $thncarirad  = $thnsekarang;
    $blncarirad  = $blnsekarang;
    $tglcarirad  = $tglsekarang;
    $thncarirad2 = $thnsekarang;
    $blncarirad2 = $blnsekarang;
    $tglcarirad2 = $tglsekarang;
    if(isset($_POST["BtnCari"])){
        $thncarirad  = validTeks(trim(isset($_POST['tgl_cari_rad']))?substr($_POST['tgl_cari_rad'],6,4):$thnsekarang);
        $blncarirad  = validTeks(trim(isset($_POST['tgl_cari_rad']))?substr($_POST['tgl_cari_rad'],3,2):$blnsekarang);
        $tglcarirad  = validTeks(trim(isset($_POST['tgl_cari_rad']))?substr($_POST['tgl_cari_rad'],0,2):$tglsekarang);
        $thncarirad2 = validTeks(trim(isset($_POST['tgl_cari_rad2']))?substr($_POST['tgl_cari_rad2'],6,4):$thnsekarang);
        $blncarirad2 = validTeks(trim(isset($_POST['tgl_cari_rad2']))?substr($_POST['tgl_cari_rad2'],3,2):$blnsekarang);
        $tglcarirad2 = validTeks(trim(isset($_POST['tgl_cari_rad2']))?substr($_POST['tgl_cari_rad2'],0,2):$tglsekarang);
    }
?>
<link href="plugins/bootstrap-datepicker/css/bootstrap-datepicker.min.css" rel="stylesheet" />
<div class="block-header">
    <h2><center>LAMA PELAYANAN RADIOLOGI</center></h2>
</div>
        ...
        <div class="card">
            <div class="body">
                <form id="form_validation" action="" method="POST">
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <label for="tgl_cari_rad">Tanggal</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" data-provide="datepicker" data-date-format="dd-mm-yyyy" pattern="(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}" title=" DD-MM-YYYY" placeholder="DD-MM-YYYY" name="tgl_cari_rad" class="datepicker form-control" required autocomplete="off" value="<?=$tglcarirad."-".$blncarirad."-".$thncarirad;?>"/>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label for="tgl_cari_rad2">Sampai Dengan</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" data-provide="datepicker" data-date-format="dd-mm-yyyy" pattern="(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}" title=" DD-MM-YYYY" placeholder="DD-MM-YYYY" name="tgl_cari_rad2" class="datepicker form-control" required autocomplete="off" value="<?=$tglcarirad2."-".$blncarirad2."-".$thncarirad2;?>"/>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <?php
                            ...
                            $lebihduajam3   = 0;
                            $jumlahData     = 0;
                            $queryMutuRad   = bukaquery(
                                "select reg_periksa.no_rkm_medis,dokter.nm_dokter,permintaan_radiologi.noorder,".
                                ...
                                "inner join permintaan_radiologi on reg_periksa.no_rawat=permintaan_radiologi.no_rawat ".
                                "where permintaan_radiologi.tgl_sampel<>'0000-00-00' and permintaan_radiologi.tgl_hasil<>'0000-00-00' ".
                                "and permintaan_radiologi.tgl_permintaan between '$thncarirad-$blncarirad-$tglcarirad' and '$thncarirad2-$blncarirad2-$tglcarirad2' ".
                                "order by permintaan_radiologi.tgl_permintaan,permintaan_radiologi.jam_permintaan"
                            );
                            while($rsqueryMutuRad = mysqli_fetch_array($queryMutuRad)) {
                                $permintaansampel = $rsqueryMutuRad["permintaansampel"];
                                $sampelhasil      = $rsqueryMutuRad["sampelhasil"];
                                $permintaanhasil  = $rsqueryMutuRad["permintaanhasil"];
                                $jumlahData++;
