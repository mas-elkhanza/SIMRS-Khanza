<?php
    ...
    $thncarirekening  = $thnsekarang;
    $blncarirekening  = $blnsekarang;
    $tglcarirekening  = $tglsekarang;
    $thncarirekening2 = $thnsekarang;
    $blncarirekening2 = $blnsekarang;
    $tglcarirekening2 = $tglsekarang;
    if(isset($_POST["BtnCari"])){
        $thncarirekening  = validTeks(trim(isset($_POST['tgl_cari_rekening']))?substr($_POST['tgl_cari_rekening'],6,4):$thnsekarang);
        $blncarirekening  = validTeks(trim(isset($_POST['tgl_cari_rekening']))?substr($_POST['tgl_cari_rekening'],3,2):$blnsekarang);
        $tglcarirekening  = validTeks(trim(isset($_POST['tgl_cari_rekening']))?substr($_POST['tgl_cari_rekening'],0,2):$tglsekarang);
        $thncarirekening2 = validTeks(trim(isset($_POST['tgl_cari_rekening2']))?substr($_POST['tgl_cari_rekening2'],6,4):$thnsekarang);
    ...
    $queryTagihan = bukaquery("select tagihan_sadewa.no_nota,tagihan_sadewa.tgl_bayar from tagihan_sadewa where tagihan_sadewa.tgl_bayar between '$thncarirekening-$blncarirekening-$tglcarirekening 00:00:00' and '$thncarirekening2-$blncarirekening2-$tglcarirekening2 23:59:59' order by tagihan_sadewa.tgl_bayar,tagihan_sadewa.no_nota");
    while($rstagihan = mysqli_fetch_array($queryTagihan)) {
        ...
        foreach($akunList as $idx => $akun) {
            if($akun["type"]=="bayar") {
                $bayarAkun = 0;
                if($kat=="rawatinap") {
                    $bayarAkun = (float) getOne("select sum(detail_nota_inap.besar_bayar) from detail_nota_inap inner join akun_bayar on detail_nota_inap.nama_bayar=akun_bayar.nama_bayar where detail_nota_inap.no_rawat='".$norawatinap."' and akun_bayar.kd_rek='".$akun["key"]."'");
                    $akunList[$idx]["rawatinap"] += $bayarAkun;
                } elseif($kat=="rawatjalan") {
                    $bayarAkun = (float) getOne("select sum(detail_nota_jalan.besar_bayar) from detail_nota_jalan inner join akun_bayar on detail_nota_jalan.nama_bayar=akun_bayar.nama_bayar where detail_nota_jalan.no_rawat='".$norawatjalan."' and akun_bayar.kd_rek='".$akun["key"]."'");
                    $akunList[$idx]["rawatjalan"] += $bayarAkun;
                } elseif($kat=="apotek") {
                    $bayarAkun = (float) getOne("select (sum(detailjual.total)+penjualan.ongkir+penjualan.ppn) from detailjual inner join penjualan on penjualan.nota_jual=detailjual.nota_jual inner join akun_bayar on penjualan.nama_bayar=akun_bayar.nama_bayar where penjualan.nota_jual='".$notajual."' and akun_bayar.kd_rek='".$akun["key"]."'");
                    $akunList[$idx]["apotek"] += $bayarAkun;
                } elseif($kat=="deposit") {
                    $bayarAkun = (float) getOne("select sum(deposit.besar_deposit) from deposit inner join akun_bayar on deposit.nama_bayar=akun_bayar.nama_bayar where deposit.no_deposit='".$nodeposit."' and akun_bayar.kd_rek='".$akun["key"]."'");
                    $akunList[$idx]["deposit"] += $bayarAkun;
                } elseif($kat=="pemasukanlain") {
                    $bayarAkun = (float) getOne("select sum(pemasukan_lain.besar) from pemasukan_lain inner join kategori_pemasukan_lain on kategori_pemasukan_lain.kode_kategori=pemasukan_lain.kode_kategori where pemasukan_lain.no_masuk='".$nopemasukanlain."' and kategori_pemasukan_lain.kd_rek2='".$akun["key"]."'");
                    $akunList[$idx]["pemasukanlain"] += $bayarAkun;
                } elseif($kat=="labkesling") {
                    $bayarAkun = (float) getOne("select sum(labkesling_detail_pembayaran_pengujian_sampel.besar_bayar) from labkesling_detail_pembayaran_pengujian_sampel inner join akun_bayar on labkesling_detail_pembayaran_pengujian_sampel.nama_bayar=akun_bayar.nama_bayar where labkesling_detail_pembayaran_pengujian_sampel.no_pembayaran='".$notakesling."' and akun_bayar.kd_rek='".$akun["key"]."'");
                ...
                $bayarPiutang = (float) getOne("select sum(detail_piutang_pasien.totalpiutang) from detail_piutang_pasien inner join akun_piutang on detail_piutang_pasien.nama_bayar=akun_piutang.nama_bayar where detail_piutang_pasien.no_rawat='".$rstagihan["no_nota"]."' and akun_piutang.kd_rek='".$akun["key"]."'");
    ...
    $queryPiutangPasien = bukaquery(
        "select piutang_pasien.no_rawat,piutang_pasien.tgl_piutang from piutang_pasien ".
        "where piutang_pasien.uangmuka=0 and piutang_pasien.tgl_piutang between '$thncarirekening-$blncarirekening-$tglcarirekening' and '$thncarirekening2-$blncarirekening2-$tglcarirekening2' ".
    ...
    while($rspp = mysqli_fetch_array($queryPiutangPasien)) {
        ...
        foreach($akunList as $idx => $akun) {
            if($akun["type"]=="piutang") {
                $bayarPiutang2 = (float) getOne("select sum(detail_piutang_pasien.totalpiutang) from detail_piutang_pasien inner join akun_piutang on detail_piutang_pasien.nama_bayar=akun_piutang.nama_bayar where detail_piutang_pasien.no_rawat='".$rspp["no_rawat"]."' and akun_piutang.kd_rek='".$akun["key"]."'");
    ...
    $queryPiutangObat = bukaquery(
        "select piutang.tgl_piutang,piutang.sisapiutang from piutang ".
        "where piutang.tgl_piutang between '$thncarirekening-$blncarirekening-$tglcarirekening' and '$thncarirekening2-$blncarirekening2-$tglcarirekening2' ".
...
<div class="block-header">
    <h2><center>PENDAPATAN PER AKUN REKENING</center></h2>
</div>
        ...
        <div class="card">
            <div class="body">
                <form id="form_validation" action="" method="POST">
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <label for="tgl_cari_rekening">Periode</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" data-provide="datepicker" data-date-format="dd-mm-yyyy" pattern="(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}" title=" DD-MM-YYYY" placeholder="DD-MM-YYYY" name="tgl_cari_rekening" class="datepicker form-control" required autocomplete="off" value="<?=$tglcarirekening."-".$blncarirekening."-".$thncarirekening;?>"/>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label for="tgl_cari_rekening2">Sampai Dengan</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" data-provide="datepicker" data-date-format="dd-mm-yyyy" pattern="(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}" title=" DD-MM-YYYY" placeholder="DD-MM-YYYY" name="tgl_cari_rekening2" class="datepicker form-control" required autocomplete="off" value="<?=$tglcarirekening2."-".$blncarirekening2."-".$thncarirekening2;?>"/>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="body" style="padding-top:0;">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        <thead>
                            <tr>
                                <th style="min-width:150px;white-space:nowrap;"><center>Akun Rekening</center></th>
            ...
            <div class="body" style="padding-top:0;">
                <div class="header bg-white" style="border-bottom:none;box-shadow:none;padding:0 20px;margin-bottom:6px;">
                    <div class="text-center" style="font-size:16px;color:#777777;">Berdasarkan Akun Rekening</div>
                </div>
                <div class="row clearfix">
                    <div class="col-md-6">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th width="70%"><center>Akun Rekening</center></th>
                    ...
                    <div class="col-md-6">
                        <div id="pie_chart_akunrekening" class="flot-chart" style="height: 400px;"></div>
                    </div>
                </div>
                <hr style="margin:6px 0 0 0;">
            </div>
            <div class="body" style="padding-top:0;">
                <div class="header bg-white" style="border-bottom:none;box-shadow:none;padding:0 20px;margin-bottom:6px;">
                    <div class="text-center" style="font-size:16px;color:#777777;">Berdasarkan Kategori Rekening</div>
                </div>
                <div class="row clearfix">
                    <div class="col-md-6">
                        <div class="table-responsive">
                    ...
                    <div class="col-md-6">
                        <div id="pie_chart_kategorirekening" class="flot-chart" style="height: 400px;"></div>
                    </div>
                </div>
                <hr style="margin:6px 0 0 0;">
            </div>
            <div class="body" style="padding-top:0;">
                <div class="header bg-white" style="border-bottom:none;box-shadow:none;padding:0 20px;margin-bottom:6px;">
                    <div class="text-center" style="font-size:16px;color:#777777;">Akun Rekening Per Tanggal</div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        <thead>
            ...
            <div class="body" style="padding-top:0;">
                <div class="header bg-white" style="border-bottom:none;box-shadow:none;padding:0 20px;margin-bottom:6px;">
                    <div class="text-center" style="font-size:16px;color:#777777;">Akun Rekening Per Bulan</div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        <thead>
            ...
            <div class="body" style="padding-top:0;">
                <div class="header bg-white" style="border-bottom:none;box-shadow:none;padding:0 20px;margin-bottom:6px;">
                    <div class="text-center" style="font-size:16px;color:#777777;">Kategori Rekening Per Tanggal</div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        <thead>
            ...
            <div class="body" style="padding-top:0;">
                <div class="header bg-white" style="border-bottom:none;box-shadow:none;padding:0 20px;margin-bottom:6px;">
                    <div class="text-center" style="font-size:16px;color:#777777;">Kategori Rekening Per Bulan</div>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        <thead>
...
$(function() {
    ...
    if (dataPieAkun.length > 0) {
        $.plot("#pie_chart_akunrekening", dataPieAkun, {
            series: {
                pie: {
                    show: true,
                    radius: 1,
        ...
        $("#pie_chart_akunrekening").html("<div class='text-center text-muted mt-5'>Kosong</div>");
    }
    var dataPieKategori = <?= json_encode($dataPieKategori) ?>;
    if (dataPieKategori.length > 0) {
        $.plot("#pie_chart_kategorirekening", dataPieKategori, {
            series: {
                pie: {
                    show: true,
                    radius: 1,
        ...
        $("#pie_chart_kategorirekening").html("<div class='text-center text-muted mt-5'>Kosong</div>");
    }
