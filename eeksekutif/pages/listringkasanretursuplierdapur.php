...
<div class="block-header">
    <h2><center>RINGKASAN RETUR SUPLIER DAPUR</center></h2>
</div>
        ...
        <div class="card">
            <div class="body">
                <form id="form_validation" action="" method="POST">
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <label for="tgl_cari_retur">Periode</label>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        ...
                        <?php
                            ...
                            $rankingJumlah = [];
                            $rankingTotal  = [];
                            $queryRetur = bukaquery(
                                "select dapur_detail_returbeli.kode_brng,dapurbarang.nama_brng,dapurbarang.jenis,kodesatuan.satuan,sum(dapur_detail_returbeli.jml_retur) as jumlah,sum(dapur_detail_returbeli.total) as total ".
                                "from dapurreturbeli inner join dapur_detail_returbeli on dapurreturbeli.no_retur_beli=dapur_detail_returbeli.no_retur_beli inner join dapurbarang on dapur_detail_returbeli.kode_brng=dapurbarang.kode_brng ".
                                "inner join kodesatuan on dapur_detail_returbeli.kode_sat=kodesatuan.kode_sat where dapurreturbeli.tgl_retur between '$thncariretur-$blncariretur-$tglcariretur' and '$thncariretur2-$blncariretur2-$tglcariretur2' ".
                                "group by dapur_detail_returbeli.kode_brng order by dapurbarang.nama_brng asc"
                            );
                            while($rsretur = mysqli_fetch_array($queryRetur)) {
                                $totalTagihan += $rsretur["total"];
                                echo "<tr>
                                        <td align='left' style='white-space:nowrap;'>".$rsretur["kode_brng"]."</td>
                                        <td align='left'>".$rsretur["nama_brng"]."</td>
                                        <td align='left' style='white-space:nowrap;'>".$rsretur["satuan"]."</td>
                                        <td align='left' style='white-space:nowrap;'>".$rsretur["jenis"]."</td>
                                        <td align='right' style='white-space:nowrap;'>".number_format($rsretur["jumlah"],0,',','.')."</td>
                                        <td align='right' style='white-space:nowrap;'>".number_format($rsretur["total"],0,',','.')."</td>
                                      </tr>";
                                $rankingJumlah[$rsretur["nama_brng"]] = (float)$rsretur["jumlah"];
                                $rankingTotal[$rsretur["nama_brng"]]  = (float)$rsretur["total"];
                            }
                        ?>
                        </tbody>
                        <tfoot>
