<?php
    ...
    $thncarilab  = $thnsekarang;
    $blncarilab  = $blnsekarang;
    $tglcarilab  = $tglsekarang;
    $thncarilab2 = $thnsekarang;
    $blncarilab2 = $blnsekarang;
    $tglcarilab2 = $tglsekarang;
    if(isset($_POST["BtnCari"])){
        $thncarilab  = validTeks(trim(isset($_POST['tgl_cari_lab']))?substr($_POST['tgl_cari_lab'],6,4):$thnsekarang);
        $blncarilab  = validTeks(trim(isset($_POST['tgl_cari_lab']))?substr($_POST['tgl_cari_lab'],3,2):$blnsekarang);
        $tglcarilab  = validTeks(trim(isset($_POST['tgl_cari_lab']))?substr($_POST['tgl_cari_lab'],0,2):$tglsekarang);
        $thncarilab2 = validTeks(trim(isset($_POST['tgl_cari_lab2']))?substr($_POST['tgl_cari_lab2'],6,4):$thnsekarang);
        $blncarilab2 = validTeks(trim(isset($_POST['tgl_cari_lab2']))?substr($_POST['tgl_cari_lab2'],3,2):$blnsekarang);
        $tglcarilab2 = validTeks(trim(isset($_POST['tgl_cari_lab2']))?substr($_POST['tgl_cari_lab2'],0,2):$tglsekarang);
    }
?>
<link href="plugins/bootstrap-datepicker/css/bootstrap-datepicker.min.css" rel="stylesheet" />
<div class="block-header">
    <h2><center>LAMA PELAYANAN LABORATORIUM PATOLOGI KLINIS</center></h2>
</div>
        ...
        <div class="card">
            <div class="body">
                <form id="form_validation" action="" method="POST">
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <label for="tgl_cari_lab">Tanggal</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" data-provide="datepicker" data-date-format="dd-mm-yyyy" pattern="(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}" title=" DD-MM-YYYY" placeholder="DD-MM-YYYY" name="tgl_cari_lab" class="datepicker form-control" required autocomplete="off" value="<?=$tglcarilab."-".$blncarilab."-".$thncarilab;?>"/>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label for="tgl_cari_lab2">Sampai Dengan</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" data-provide="datepicker" data-date-format="dd-mm-yyyy" pattern="(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}" title=" DD-MM-YYYY" placeholder="DD-MM-YYYY" name="tgl_cari_lab2" class="datepicker form-control" required autocomplete="off" value="<?=$tglcarilab2."-".$blncarilab2."-".$thncarilab2;?>"/>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <?php
                            ...
                            $lebihduajam3   = 0;
                            $jumlahData     = 0;
                            $queryMutuLab   = bukaquery(
                                "select reg_periksa.no_rkm_medis,dokter.nm_dokter,permintaan_lab.noorder,".
                                ...
                                "where permintaan_lab.tgl_sampel<>'0000-00-00' and permintaan_lab.tgl_hasil<>'0000-00-00' ".
                                "and permintaan_lab.tgl_permintaan between '$thncarilab-$blncarilab-$tglcarilab' and '$thncarilab2-$blncarilab2-$tglcarilab2' ".
                                "order by permintaan_lab.tgl_permintaan,permintaan_lab.jam_permintaan"
                            );
                            while($rsqueryMutuLab = mysqli_fetch_array($queryMutuLab)) {
                                $permintaansampel = $rsqueryMutuLab["permintaansampel"];
                                $sampelhasil      = $rsqueryMutuLab["sampelhasil"];
                                $permintaanhasil  = $rsqueryMutuLab["permintaanhasil"];
                                $jumlahData++;
