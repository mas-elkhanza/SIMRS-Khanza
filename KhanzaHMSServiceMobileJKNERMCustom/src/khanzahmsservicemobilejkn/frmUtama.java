                ...
                if(detik.equals("01")&&((nilai_menit%5)==0)){
                    ...
                    try {
                        ...
                        ps=koneksi.prepareStatement(
                                ...
                                "AND reg_periksa.kd_poli IN ( SELECT maping_poli_bpjs.kd_poli_rs FROM maping_poli_bpjs WHERE maping_poli_bpjs.kd_poli_rs <> 'LB001' AND maping_poli_bpjs.kd_poli_rs <> 'IGDK') "+
                                "ORDER BY reg_periksa.kd_dokter, reg_periksa.no_reg ASC");
                        try {
                            rs=ps.executeQuery();
                            while(rs.next()){
                                try {
                                    TeksArea.append("POLI : "+rs.getString("kd_poli") + "\n");
                                    System.out.println(hari);
                                    System.out.println(rs.getString("kd_dokter"));
                                    ...
                                    if(rs2.next()){
                                        TeksArea.append("POLI Yang dikirim: "+rs2.getString("kd_poli") + "\n");
                                                ...
                                                if(!datajam.equals("")){
                                                    if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{rs.getString("no_rawat"),"3",datajam})==true){
                                                        ...
                                                        try {     
                                                            ...
                                                            TeksArea.append("respon WS BPJS untuk kode booking "+rs.getString("no_rawat")+" : "+nameNode.path("code").asText()+" "+nameNode.path("message").asText()+"\n");
                                                        }catch (Exception ex) {
...
//                                                        parsedDate = dateFormat.parse(datajam);
//                                                        try {     
//                                                            TeksArea.append("Menjalankan WS taskid batal pelayanan poli BPJS Onsite\n");
//                                                            headers = new HttpHeaders();
//                                                            headers.setContentType(MediaType.APPLICATION_JSON);
//                                                            headers.add("x-cons-id",koneksiDB.CONSIDAPIMOBILEJKN());
//                                                            utc=String.valueOf(api.GetUTCdatetimeAsString());
