    ...
    <div class="entry">        
        <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
	        ...
	        <tr class="head">
                ...
                <tr class="head">
                    <td width="31%" >File Gambar Treadmill</td><td width="">:</td>
                    <td width="67%">
                        ...
                        if(($_FILES['gambar']['type'] == 'image/jpeg')||($_FILES['gambar']['type'] == 'image/jpg')){
                            if((@mime_content_type($_FILES['gambar']['tmp_name'])== 'image/jpeg')||(@mime_content_type($_FILES['gambar']['tmp_name'])== 'image/jpg')){
                                if((!empty($no_rawat))&&(!empty($gambar))) {
                                    if(Tambah(" hasil_pemeriksaan_treadmill_gambar "," '$no_rawat','$gambar'", " Gambar EKG " )){
            ...
            <?php
                $_sql = "SELECT * from hasil_pemeriksaan_treadmill_gambar where hasil_pemeriksaan_treadmill_gambar.no_rawat='$no_rawat'";
        ...
        <?php
            if ($action=="HAPUS") {
                unlink($_GET['gambar']);
                Hapus(" hasil_pemeriksaan_treadmill_gambar "," no_rawat ='".validTeks4($_GET['no_rawat'],20)."'","?act=List&action=TAMBAH&no_rawat=$no_rawat");
            }
