...
<div id="post">
    <div class="entry">
		...
		$_sql               = "SELECT pegawai.nik,pegawai.nama FROM pegawai where pegawai.id='$id'";
                $hasil              = bukaquery($_sql);
                $baris              = mysqli_fetch_row($hasil);  
                $_sqlnext           = "SELECT pegawai.id FROM pegawai WHERE pegawai.id>'$id' and pegawai.jbtn like '%dokter umum%' order by pegawai.id asc limit 1";
                $hasilnext          = bukaquery($_sqlnext);
                $barisnext          = mysqli_fetch_row($hasilnext);
                @$next              = $barisnext[0];
                $_sqlprev           = "SELECT pegawai.id FROM pegawai WHERE pegawai.id<'$id' and pegawai.jbtn like '%dokter umum%' order by pegawai.id desc limit 1";
		...
		<tr class="head">
                    ...
                    <td width="75%">
                        <select name="tnd" class="text2" onkeydown="setDefault(this, document.getElementById('MsgIsi1'));" id="TxtIsi1" autofocus>
                            <?php
                                $_sql = "SELECT master_tindakan.id,master_tindakan.nama FROM master_tindakan where master_tindakan.jns='dr Umum' ORDER BY master_tindakan.nama";
                                $hasil=bukaquery($_sql);
